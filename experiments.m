...
parameters["e"] := 40;
parameters["e_inc"] := 20;
parameters["max_e"] := 250;
parameters["max_prec"] := 30;
/*
...
function extras(curve_data, parameters)
    ...
    g := curve_data[3]; // defines non-tor point
    if Degree(g) eq 1 then
    	p := 5;
    ...
    success, chabauty_data := effective_chabauty_incremental(f, g, p, parameters);
    if success then
        L := chabauty_data[1];
        ...
        return success, extras_data, Qpoints, b_values, [p, N, e];
    else
        return false, [], [], [], [];
    end if;
end function;
...
procedure batch_extras(curves_path, extras_path, extras_with_b_path)
    ...
    while true do
        ...
        if IsEof(s) then
            ...
            curve_data := curve_line_parser(s);
            success, extras_data, Qpoints, b_values, params := extras(curve_data, parameters);
            ...
            if success then
                ...
                Qpoints := strip(Sprint(picard_points(curve_data[2], parameters["height"])));
                Puts(extras_file, s cat ":" cat Qpoints cat ":" cat extras_data cat ":" cat params);
                Puts(extras_with_b_file, s cat ":" cat Qpoints cat ":" cat extras_data cat ":" cat params cat ":" cat b_values);
            else
